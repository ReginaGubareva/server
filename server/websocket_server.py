...
from server.thread import WebsocketServerThread
...
class WebsocketServer(ThreadingMixIn, TCPServer, API):
    ...
    @staticmethod
    def _terminate_client_handler(handler):
    ...
    def __init__(self, socket, addr, server):
        ...
        if server.key and server.cert:
            try:
                ...
                logger.warning(
                    "SSL not available (are the paths {} and {} correct for the key and cert?)".format(server.key,
                                                                                                       server.cert))
